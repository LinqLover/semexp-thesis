...
for example, when writing Smalltalk code, GPT-4o often produces syntax errors and shows insufficient knowledge of standard libraries such as the \code{Collections} package.
...
for example, because GPT-4o often forgets necessary brackets when chaining message sends, we extend all \code{MessageNotUnderstood} errors from the system with an explaining comment and provide an example of correcting an incorrect message chain, which the model then considers in the next attempt.
