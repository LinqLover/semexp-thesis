...
\begin{example}
	...
	Only when they are forced to manually translate ``filed'' into technical terms of the order interface---such as \code{creationDate}, \code{submissionDate}, or \code{receiptDate}---, they might realize that there are multiple options and start thinking about what information exactly they want to display to users.
...
In our concept of the semantic workspace, we explored different gradations between augmentation and automation for semantic tools, both of which intend different cooperation styles between the programmer and the system~(see \cref{sec:approach/workspace}). % TODO: ever "have" (perfect)?
...
Even when the accuracy of semantic technologies evolves in the future, the law of leaky abstraction\footref{fn:joels_law} suggests that the success of semantic tools will remain uncertain.
...
\subsection{Performance Limitations: Threats to Temporal Immediacy and Financial Budgets}
