...
Between both extremes, gradations exist: for example, the exploratory programming agent might request senders of a method in the context of a broader topic, such as users of the \code{WriteStream>>nextPut:} method in Squeak that are related to formatting dates.
...
\begin{description}
	...
	\item[Diversity]
	A ranking must contain diverse suggestions that display different highly types and topics from the corpus.

	We formalize the diversity of a ranking by adding all mutual distances between two suggestions.
...
\begin{figure}
	...
	\caption[Different methods for semantic ranking achieve different trade-offs between our defined objectives.]{
		...
		%Probabilistic sampling maximizes diversity and representativeness but does ignores the relevance or redundance of items.
