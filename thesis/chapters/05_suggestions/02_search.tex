...
For this, we embed each query (i.e., each input artifact) using a text embedding model\footnote{In our prototype, we use OpenAI's \code{text-embedding-3-large} model for embedding any artifacts with a reduced dimension size of 256~(see \cref{sec:discussion/performance/memory}).}, search a vector database of all other available artifact (such as methods in the system) for documents with close embeddings, and return the top $k$ most similar results.
...
first, computing and storing embeddings involves noticeable costs in terms of time, storage, and financial expenses~(see \cref{sec:discussion/performance}).
...
This approach yields a satisfying quality of results and a satisfying performance of search (typically 250 milliseconds for \qty{25}{MB} of source code) even without maintaining an index.
